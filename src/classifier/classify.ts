...
export function classifyComplexity(messages: Message[]): ClassificationResult {
  ...
  // Early-exit for simple greetings — check if "hi"/"hello" etc appears ANYWHERE in short content
...
function analyzeHistoricalContext(messages: Message[]): string[] {
  ...
  if (userMessages.length > 1) {
    ...
    if (lengths.length >= 2) {
      const earlyAvg =
        lengths.slice(0, Math.ceil(lengths.length / 2)).reduce((a, b) => a + b, 0) /
        Math.ceil(lengths.length / 2);
      const lateAvg =
        lengths.slice(Math.floor(lengths.length / 2)).reduce((a, b) => a + b, 0) /
        Math.ceil(lengths.length / 2);
  ...
  const toolMessages = messages.filter(
    (m) => m.role === 'tool' || m.tool_calls?.length || m.tool_use?.length,
  ).length;
...
function applyConversationContext(
  ...
  const totalAdjustment =
    Object.values(adjustedScores).reduce((a, b) => a + b, 0) -
    Object.values(scoringResult.scores).reduce((a, b) => a + b, 0);

  if (Math.abs(totalAdjustment) > 0.1) {
    // Renormalize
    const sum =
      adjustedScores.simple +
      adjustedScores.mid +
      adjustedScores.complex +
      adjustedScores.reasoning;
    adjustedScores = {
